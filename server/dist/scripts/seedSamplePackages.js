...
const seedSamplePackages = async () => {
    try {
        await connectDB();
        let testUser = await User_1.default.findOne({ email: 'test2@mailtracker.com' });
        if (!testUser) {
            testUser = await User_1.default.findOne({ email: 'test@mailtracker.com' });
        }
        if (!testUser) {
            console.log('Test user not found. Please create a test user first.');
            console.log('Available users:');
            const users = await User_1.default.find({}, 'email username');
            users.forEach(user => console.log(`- ${user.email} (${user.username})`));
            process.exit(1);
        }
        console.log(`Using test user: ${testUser.email}`);
        const existingPackages = await Package_1.default.find({ userId: testUser._id });
        ...
        await Package_1.default.insertMany(samplePackages);
...
seedSamplePackages();
//# sourceMappingURL=seedSamplePackages.js.map